...
{
	public bool curvedDisplay = false;
	public GameObject curvedDisplayObject;
    ...
    public void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList, Ray ray, bool checkForBlocking)
    {
		...
		// replace ray with a ray from curved display
		if (curvedDisplay && curvedDisplayObject!=null) {
			Ray incomingRay = eventData.GetRay ();

			var hits = Physics.RaycastAll (incomingRay);
			foreach(var hit in hits) {
				if (hit.transform.gameObject == curvedDisplayObject) {
